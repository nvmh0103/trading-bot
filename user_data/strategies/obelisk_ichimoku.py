    ...
    def fast_tf_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        if self.config['runmode'].value == 'hyperopt':
            for len in range(30, 91):
                dataframe[f'zema_{len}'] = ftt.dema(dataframe, period=len)
        else:
            ...
            dataframe[f'zema_buy'] = ftt.dema(dataframe, period=self.zema_len_buy.value) * self.low_offset.value
            dataframe[f'zema_sell'] = ftt.dema(dataframe, period=self.zema_len_sell.value) * self.high_offset.value
